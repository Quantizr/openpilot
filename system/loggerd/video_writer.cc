...
VideoWriter::VideoWriter(const char *path, const char *filename, bool remuxing, int width, int height, int fps, cereal::EncodeIndex::Type codec, bool has_audio)
  ...
  if (this->remuxing) {
    ...
    if (has_audio) {
      ...
      // Create audio stream for muxing pre-encoded AAC data
      this->audio_stream = avformat_new_stream(this->ofmt_ctx, NULL);
      assert(this->audio_stream);

      // Set up audio stream parameters for AAC (matches AudioEncoder output)
      this->audio_stream->codecpar->codec_type = AVMEDIA_TYPE_AUDIO;
      this->audio_stream->codecpar->codec_id = AV_CODEC_ID_AAC;
      this->audio_stream->codecpar->sample_rate = 16000; // from system/micd.py
      this->audio_stream->codecpar->bit_rate = 32000;
      #if LIBAVUTIL_VERSION_INT >= AV_VERSION_INT(57, 28, 100) // FFmpeg 5.1+
      av_channel_layout_default(&this->audio_stream->codecpar->ch_layout, 1);
      #else
      this->audio_stream->codecpar->channel_layout = AV_CH_LAYOUT_MONO;
      #endif
      this->audio_stream->time_base = (AVRational){1, 16000}; // sample rate
...
void VideoWriter::write_encoded_audio(uint8_t *data, int len, long long timestamp) {
  if (!remuxing || !audio_stream || !data || len <= 0) return;
  AVPacket pkt;
  av_init_packet(&pkt);
  pkt.data = data;
  pkt.size = len;
  ...
  pkt.pts = pkt.dts = av_rescale_q(timestamp, in_timebase, audio_stream->time_base);

  int err = av_interleaved_write_frame(ofmt_ctx, &pkt);
  if (err < 0) {
    LOGW("AUDIO: Write encoded frame failed - error: %d", err);
  }
